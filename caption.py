...
def caption_image_beam_search_noAtt(encoder, decoder, imagePath, wordMap, beamSize=3):
    """
    Reads an image and captions it with beam search.
    :param encoder: encoder model
    :param decoder: decoder model
    ...
    while True:
        embeddings = decoder.embedding(kPrevWords).squeeze(1)  # (k, embed_dim)
        h, c = decoder.decode_step(embeddings, (h, c))  # (k, decoder_dim)
...
def caption_image_beam_search_transformer(encoder, decoder, imagePath, wordMap, beamSize=3, max_decode_len= 51):
    ...
    completeSeqsScores = list()
    step = 0 
    finishedSequences = torch.zeros(k, dtype=torch.bool, device=device) # False for all beams initially
    ...
    while True:
        ...
        kPrevWordsActive = kPrevWords[active] # (active_k, current_seq_len)
        ...
        decoderOutput = decoder.transformer_decoder(
            tgtActive,                             # [current_seq_len, k, embed_dim]
            encoderOutActive,                # [num_pixels, k, embed_dim] (full k)
            tgt_mask=tgtMask) 
        
        lastTokenOutputActive = decoderOutput[-1, :, :] # Shape: [active_k, embed_dim]
        # Project to vocabulary size to get logits
        scoresActive = decoder.fc_out(lastTokenOutputActive) # [active_k, vocab_size]
        scoresActive = F.log_softmax(scoresActive, dim=1) # Convert to log-probabilities
        ...
        if step == 0: 
            topKScoresNew, topKUnrolledIndices = scoresActive[0].topk(k, 0, True, True) # (k)
        else:
            topKScoresNew, topKUnrolledIndices = scoresActive.view(-1).topk(k, 0, True, True) # (k)
...
def caption_image_beam_search_transformer_attention(encoder, decoder, imagePath, wordMap, filename, beamSize=3, max_decode_len=51):
    ...
    while True:
        ...
        tgtActive = embeddingsActive.permute(1, 0, 2)
        tgtMask = nn.Transformer.generate_square_subsequent_mask(tgtActive.size(0)).to(device).bool()
        currentLayerOutput = tgtActive # Input to the first decoder layer for active sequences
        allLayerCrossAttentionsForStep = [] # To collect cross-attention from each layer for *this step 't'*
        ...
        for layer_idx, layer in enumerate(decoder.decoder_layers): # Iterate through layers (nn.ModuleList of CustomTransformerDecoderLayer)
            layer_output, self_attn_weights, cross_attn_weights_current_layer = layer(
                currentLayerOutput, 
                encoderOutActive,      # Sliced encoder_memory
                tgt_mask=tgtMask,
                output_attentions=True # Request attention weights here
            )
            currentLayerOutput = layer_output # Update output for next layer (input to next layer)
        ...
        # crossAttnForCurrentToken = stackedCrossAttentions[:, stackedCrossAttentions.size(1) - 1, :, :, :] 
    ...
    if len(completeSeqs) == 0: 
        bestSeqScoreIdx = topKScores.argmax()
        finalSeq = kPrevWordsIds[bestSeqScoreIdx].tolist()
        finalAlphas = seqsAlphas[bestSeqScoreIdx].tolist()
    else:
        bestSeqScoreIdx= completeSeqsScores.index(max(completeSeqsScores))
        finalSeq = completeSeqs[bestSeqScoreIdx]
        finalAlphas = completeSeqsAlphas[bestSeqScoreIdx]

    return finalSeq, finalAlphas
