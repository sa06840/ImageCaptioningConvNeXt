...
def create_input_files(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
    ...
    for impaths, imcaps, split in [(train_image_paths, train_image_captions, 'TRAIN'),
        ...
        with h5py.File(os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'), 'a') as h:   # This opens an HDF5 file for storing images in the current split (train/val/test).
            # Make a note of the number of captions we are sampling per image
            h.attrs['captions_per_image'] = captions_per_image

            # Create dataset inside HDF5 file to store images (The images dataset is created to store the images as 3x256x256 arrays)
            ...
            for i, path in enumerate(tqdm(impaths)):  # tqdm(impaths) is a progress bar that shows how much of the list has been processed
                ...
                if len(imcaps[i]) < captions_per_image:
                    captions = imcaps[i] + [choice(imcaps[i]) for _ in range(captions_per_image - len(imcaps[i]))]  # If the image has fewer captions than needed, the code will randomly duplicate captions from the existing ones
                else:
                    captions = sample(imcaps[i], k=captions_per_image)  # If the image has enough captions (5 or greater), it will randomly sample the required number of captions
...
def save_checkpoint(dataName, epoch, epochsSinceImprovement, encoderSaved, decoderSaved, encoderOptimizer, decoderOptimizer,
                    bleu4, isBest, results, lstmDecoder):
    """
    ...
    if lstmDecoder is True:
        ...
        filename = 'checkpoint_HFTransformer_' + dataName + '.pth.tar'
